...
| Codex-created illustration profile mappings, PDF analysis outcome tables, migration imports | `../docs/agents/illustration-profile-codex-training.md`, `db/migrations/012_illustration_profiles.sql` | `src/types/illustration.ts`, `src/services/illustrationMappingEngine.ts`, `src/services/illustrationRuntimeExtraction.ts`, `src/services/illustrationMatching.ts` | `manle-illustration-profile-codex-training`, `Profile Key`, `Field Path`, `Projection Key`, `source_strategy`, `fingerprint_type`, `profile_version_id` |
